...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(9):
        if len(set(get_row(solution, (i, 0)))) != 9 or get_row(solution, (i, 0)).count('.') != 0:
            return False
    for i in range(9):
        if len(set(get_col(solution, (0, i)))) != 9 or get_col(solution, (0, i)).count('.') != 0:
            return False
    for i in range(0, 9, 3):
        for j in range(0, 9, 3):
            if len(get_block(solution, (i, j))) != 9 or get_block(solution, (i, 0)).count('.') != 0:
                return False
    return True
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    if N > 80:
        return grid
    random_nums = []
    while len(random_nums) < 81-N:
        my_tuple = (random.randint(0, 8), random.randint(0, 8))
        if my_tuple not in random_nums:
            random_nums.append(my_tuple)
